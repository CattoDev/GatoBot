...
std::string FFmpegSettings::getCommandStr() {
    ...
    // codec
    if (extraArgs.find("-c:v") == extraArgs.npos && !codec.empty())
        stream << " -c:v " << codec;

    // bitrate
    if (extraArgs.find("-b:v") == extraArgs.npos && bitrate > 0)
        stream << " -b:v " << bitrate << "k";
...
std::string FFmpegSettings::getSongCmdStr() {
    std::stringstream stream;
